...
- Encrypted secret placeholders live close to the workloads:
  ...
  - `manifests/service/secrets.yaml`: `auth-api-env`, `registry-admin-env`, `packages-verdaccio-env`, `packages-verdaccio-auth`, `packages-ghcr-pull`, `sre-api-env` (key: `DATABASE_URL_AUTH` — DSN to auth Postgres)
