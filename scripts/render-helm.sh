...
helm lint "${ROOT_DIR}/charts/app/sre"
...
helm template app-sre "${ROOT_DIR}/charts/app/sre" --namespace service > "${TMP_DIR}/sre.yaml"
...
mv "${TMP_DIR}/sre.yaml" "${ROOT_DIR}/manifests/service/sre.yaml"
...
echo "Rendered Helm manifests into manifests/service/{auth,smoke,registry,sre,packages,crates}.yaml"
